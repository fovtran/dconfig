...
static void terremoto(GstBus *bus, GstMessage *message, gpointer user_data)
{
  /* also clean up */
...
static void on_message_async_done (GstBus *bus, GstMessage *message, gpointer user_data)
{
  ...
  if (!is_buffering)
    ...
    g_timeout_add (1, buffer_timeout, pipeline);
}
...
gint main (gint   argc,  gchar *argv[])
{
  ...
  // g_signal_connect (bus, "message::playback-done  ", (GCallback) buffer_timeout, pipeline);
