...
function ingests()
{
curl -H 'Accept: application/vnd.twitchtv.v5+json' -H 'Client-ID: $STREAM_KEY' -X GET 'https://api.twitch.tv/kraken/ingests'
}
...
#HWACCELAPI="-hwaccel vaapi -hwaccel_output_format vaapi -f drm"
...
function daemon()
{
/usr/bin/ffmpeg -hide_banner -init_hw_device vaapi=/dev/dri/renderD129 $HWACCELAPI -s $INRES -r $FPS -threads 4 -probesize 24M -thread_queue_size 16 -i $I $PULSEAUDIO \
	-vcodec $VIDEOCODEC -g $GOP -keyint_min $GOPMIN -b:v $CBR -minrate $CBR \
	-maxrate $CBR -vf "scale=$OUTRES:-1,format=yuv420p" \
	-preset $QUALITY -acodec $AUDIOCODEC \
 	-bufsize $CBR -threads $THREADS -strict normal \
	-safe 0 -segment_time_metadata 1 -f $OUTFMT $TARGET 	2>&1
}
exit 0
